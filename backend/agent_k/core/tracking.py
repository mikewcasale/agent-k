...
# Word-boundary prefix `(?<!\w)` ensures each parameter name is captured only
# when it is not a suffix of a longer identifier, so `alpha` will not match inside
# `quantile_alpha`, `subsample` will not match inside `feature_subsample`, etc.
_HYPERPARAM_PATTERNS: Final[dict[str, re.Pattern[str]]] = {
    "n_estimators": re.compile(r"(?<!\w)(n_estimators\s*=\s*)(\d+)", re.IGNORECASE),
    "learning_rate": re.compile(r"(?<!\w)(learning_rate\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "max_depth": re.compile(r"(?<!\w)(max_depth\s*=\s*)(\d+)", re.IGNORECASE),
    "min_child_samples": re.compile(r"(?<!\w)(min_child_samples\s*=\s*)(\d+)", re.IGNORECASE),
    "num_leaves": re.compile(r"(?<!\w)(num_leaves\s*=\s*)(\d+)", re.IGNORECASE),
    "subsample": re.compile(r"(?<!\w)(subsample\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "colsample_bytree": re.compile(r"(?<!\w)(colsample_bytree\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "feature_fraction": re.compile(r"(?<!\w)(feature_fraction\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "bagging_fraction": re.compile(r"(?<!\w)(bagging_fraction\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "bagging_freq": re.compile(r"(?<!\w)(bagging_freq\s*=\s*)(\d+)", re.IGNORECASE),
    "min_samples_leaf": re.compile(r"(?<!\w)(min_samples_leaf\s*=\s*)(\d+)", re.IGNORECASE),
    "min_samples_split": re.compile(r"(?<!\w)(min_samples_split\s*=\s*)(\d+)", re.IGNORECASE),
    "max_features": re.compile(r"(?<!\w)(max_features\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "n_neighbors": re.compile(r"(?<!\w)(n_neighbors\s*=\s*)(\d+)", re.IGNORECASE),
    "leaf_size": re.compile(r"(?<!\w)(leaf_size\s*=\s*)(\d+)", re.IGNORECASE),
    "p": re.compile(r"(?<!\w)(p\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "weights": re.compile(r"(?<!\w)(weights\s*=\s*)(\"[^\"]+\"|'[^']+'|\w+)", re.IGNORECASE),
    "metric": re.compile(r"(?<!\w)(metric\s*=\s*)(\"[^\"]+\"|'[^']+'|\w+)", re.IGNORECASE),
    "algorithm": re.compile(r"(?<!\w)(algorithm\s*=\s*)(\"[^\"]+\"|'[^']+'|\w+)", re.IGNORECASE),
    "objective": re.compile(r"(?<!\w)(objective\s*=\s*)(\"[^\"]+\"|'[^']+'|\w+)", re.IGNORECASE),
    "huber_delta": re.compile(r"(?<!\w)(huber_delta\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "quantile_alpha": re.compile(r"(?<!\w)(quantile_alpha\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "lambda_l1": re.compile(r"(?<!\w)(lambda_l1\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "lambda_l2": re.compile(r"(?<!\w)(lambda_l2\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "min_split_gain": re.compile(r"(?<!\w)(min_split_gain\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "min_child_weight": re.compile(r"(?<!\w)(min_child_weight\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "max_bin": re.compile(r"(?<!\w)(max_bin\s*=\s*)(\d+)", re.IGNORECASE),
    "alpha": re.compile(r"(?<!\w)(alpha\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "l1_ratio": re.compile(r"(?<!\w)(l1_ratio\s*=\s*)([\d\.]+)", re.IGNORECASE),
    "max_iter": re.compile(r"(?<!\w)(max_iter\s*=\s*)(\d+)", re.IGNORECASE),
}
