...
def _fitness_or(individual: Individual[Any], sentinel: float) -> float:
    """Return ``individual.fitness`` if evaluated, else ``sentinel``.

    Explicit ``is None`` check because ``0.0`` and ``-0.0`` are legitimate
    fitness values that Python's ``or`` short-circuit would coerce to the
    sentinel.
    """
    fitness = individual.fitness
    return sentinel if fitness is None else fitness
...
class MapElitesArchive(Generic[GenomeT]):
    ...
    def add(self, individual: Individual[GenomeT]) -> None:
        ...
        if current is None or _fitness_or(current, -math.inf) < _fitness_or(individual, -math.inf):
    ...
    def sample(self, *, top: int = 1, diverse: int = 1) -> list[Individual[GenomeT]]:
        ...
        if not entries:
            return []
        sorted_entries = sorted(entries, key=lambda item: _fitness_or(item, -math.inf), reverse=True)
    ...
    def _trim_archive(self) -> None:
        if not self._max_cells:
            return
        entries = sorted(self._cells.items(), key=lambda item: _fitness_or(item[1], -math.inf), reverse=True)
...
class Population(Generic[GenomeT]):
    ...
    def best(self) -> Individual[GenomeT] | None:
        ...
        if not self.individuals:
            return None
        return max(self.individuals, key=lambda item: _fitness_or(item, -math.inf))
    ...
    def tournament(self, *, size: int = 3) -> Individual[GenomeT]:
        ...
        return max(candidates, key=lambda item: _fitness_or(item, -math.inf))
    ...
    def evolve_generation(
        ...
        *,
        ...
        ranked = sorted(self.individuals, key=lambda item: _fitness_or(item, -math.inf), reverse=True)
