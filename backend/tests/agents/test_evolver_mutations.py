...
try:
    from agent_k.agents.evolver import (
        _MODEL_SWAPS,
        _apply_crossover,
        _apply_hyperparameter_mutation,
        _apply_point_mutation,
        _apply_structural_mutation,
        _fitness_from_score,
        _seeded_rng,
    )
...
class TestFitnessFromScore:
    ...
    def test_fitness_from_score(self, score: float, direction: str, expected: float) -> None:
        ...
        assert _fitness_from_score(score, direction) == pytest.approx(expected)
...
class TestSeededRng:
    ...
    def test_seeded_rng_is_deterministic(self) -> None:
        ...
        rng_a = _seeded_rng("code", {"a": 1, "b": 2}, "salt")
        rng_b = _seeded_rng("code", {"a": 1, "b": 2}, "salt")
        rng_c = _seeded_rng("code", {"a": 1, "b": 2}, "other")
...
class TestPointMutation:
    ...
    def test_point_mutation_respects_max_changes(self) -> None:
        ...
        mutated = _apply_point_mutation(code, params)
        numbers_before = re.findall(r"-?\d+\.?\d*", code)
        numbers_after = re.findall(r"-?\d+\.?\d*", mutated)

        differences = sum(
            1 for before, after in zip(numbers_before, numbers_after) if before != after
        )
...
class TestHyperparameterMutation:
    ...
    def test_hyperparameter_mutation_keeps_integer(self) -> None:
        ...
        mutated = _apply_hyperparameter_mutation(code, {"magnitude": 0.3})
    ...
    def test_hyperparameter_mutation_updates_float(self) -> None:
        ...
        mutated = _apply_hyperparameter_mutation(code, {"magnitude": 0.5})
...
class TestStructuralMutation:
    ...
    def test_structural_mutation_model_swap(self, source: str, target: str) -> None:
        ...
        code = f"model = {source}()"
        mutated = _apply_structural_mutation(code, {})
...
class TestCrossover:
    ...
    def test_crossover_merges_imports(self) -> None:
        ...
        merged = _apply_crossover(code, other, {})
