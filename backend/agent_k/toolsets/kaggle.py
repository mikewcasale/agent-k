...
def with_tool_telemetry(
    *, task_id: str, tool_type: str, operation: str, error_response: Callable[[str], ToolResultT], result_summary: Callable[[ToolResultT], dict[str, Any]]
    ...
    def decorator(func: Callable[P, Awaitable[ToolResultT]]) -> Callable[P, Awaitable[ToolResultT]]:
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> ToolResultT:
            ...
            await _emit_tool_event(ctx, "emit_tool_start", task_id=task_id, tool_call_id=tool_call_id, tool_type=tool_type, operation=operation)

            try:
                result = await func(*args, **kwargs)
            except Exception as exc:
                await _emit_tool_event(ctx, "emit_tool_error", task_id=task_id, tool_call_id=tool_call_id, tool_type=tool_type, operation=operation, error=str(exc))
                return error_response(str(exc))

            duration_ms = int((time.perf_counter() - start_time) * 1000)
            await _emit_tool_event(
                ctx, "emit_tool_result", task_id=task_id, tool_call_id=tool_call_id, tool_type=tool_type, operation=operation, result=result_summary(result), duration_ms=duration_ms
            )
            return result
...
# =============================================================================
@kaggle_toolset.tool
@with_tool_telemetry(task_id="kaggle_search", tool_type="kaggle_mcp", operation="competitions.list", error_response=_error_list_response, result_summary=_search_summary)
async def kaggle_search_competitions(
    ctx: RunContext[Any], categories: list[str] | None = None, keywords: list[str] | None = None, min_prize: int | None = None, active_only: bool = True
) -> list[dict[str, Any]]:
    ...
    with logfire.span("kaggle_search_competitions", categories=categories, keywords=keywords):
        ...
        async for comp in adapter.search_competitions(categories=categories, keywords=keywords, min_prize=min_prize, active_only=active_only):
...
@kaggle_toolset.tool
@with_tool_telemetry(task_id="kaggle_competition", tool_type="kaggle_mcp", operation="competitions.get", error_response=_error_dict_response, result_summary=_competition_summary)
...
@kaggle_toolset.tool
@with_tool_telemetry(task_id="kaggle_leaderboard", tool_type="kaggle_mcp", operation="competitions.leaderboard", error_response=_error_dict_response, result_summary=_leaderboard_summary)
async def kaggle_get_leaderboard(ctx: RunContext[Any], competition_id: str, limit: int = 20) -> dict[str, Any]:
    ...
    with logfire.span("kaggle_get_leaderboard", competition_id=competition_id):
        ...
        return {"competition_id": competition_id, "total_entries": len(entries), "entries": [{"rank": e.rank, "team_name": e.team_name, "score": e.score} for e in entries]}


@kaggle_toolset.tool
@with_tool_telemetry(task_id="kaggle_datasets", tool_type="kaggle_mcp", operation="competitions.data", error_response=_error_dict_response, result_summary=_dataset_summary)
async def kaggle_list_datasets(ctx: RunContext[Any], competition_id: str) -> dict[str, Any]:
    ...
    with logfire.span("kaggle_list_datasets", competition_id=competition_id):
        ...
        files = response.json()
        return {"competition_id": competition_id, "files": [{"name": f.get("name"), "size": f.get("totalBytes"), "description": f.get("description")} for f in files]}
