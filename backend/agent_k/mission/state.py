...
class MissionState(BaseModel):
    ...
    competition_id: CompetitionId | None = Field(
        default=None, description="Selected competition id"
    )

    # Configuration
    criteria: MissionCriteria = Field(
        default_factory=MissionCriteria, description="Mission selection criteria"
    )

    # Phase tracking
    current_phase: MissionPhase = Field(default="discovery", description="Current mission phase")
    phases_completed: list[MissionPhase] = Field(
    ...
    discovered_competitions: list[Competition] = Field(
        ...
        description="Competitions found during discovery",
    )
    selected_competition: Competition | None = Field(
        default=None,
        description="Competition selected for execution",
    )
    research_findings: ResearchFindings | None = Field(
        default=None,
        description="Research findings from scientist phase",
    )
    prototype_code: str | None = Field(
        default=None,
        description="Prototype solution code",
    )
    prototype_score: float | None = Field(
        default=None,
        description="Prototype evaluation score",
    )
    evolution_state: EvolutionState | None = Field(
        default=None,
        description="Evolution phase state",
    )
    final_submission_id: str | None = Field(
        default=None,
        description="Final submission identifier",
    )
    final_score: float | None = Field(
        default=None,
        description="Final leaderboard score",
    )
    final_rank: int | None = Field(
        default=None,
        description="Final leaderboard rank",
    )
    ...
    memory: MemoryState = Field(
        ...
        description="Mission memory state",
    )

    # Planning (for frontend)
    phases: list[PhasePlan] = Field(
        default_factory=list,
        description="Phase plans for UI display",
    )
    current_task_id: TaskId | None = Field(
        default=None,
    ...
    overall_progress: float = Field(
        ...
        ge=0.0,
        le=100.0,
        description="Overall mission progress (percentage)",
    )
...
class MissionResult(BaseModel):
    ...
    total_submissions: int = Field(
        default=0,
        description="Number of submissions made",
    )
    evolution_generations: int = Field(
        default=0,
        description="Evolution generations completed",
    )
    ...
    phases_completed: list[MissionPhase] = Field(
        default_factory=list,
        description="Phases completed during mission",
    )
    error_message: str | None = Field(
        default=None,
        description="Error message when mission fails",
    )
