    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <StatCard
          hint={
            latestGeneration
              ? `Mean ${latestGeneration.meanFitness.toFixed(4)}`
              : undefined
          }
          icon={Trophy}
          label="Best Fitness"
        ...
        <StatCard
          hint={
            evolution.convergenceDetected
              ? evolution.convergenceReason
              : "Active"
          }
          icon={Zap}
          label="Population"
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mb-3 flex items-center justify-between">
          ...
          {state.mission.research?.leaderboardAnalysis?.targetScore && (
            <span className="rounded-full bg-amber-100 px-3 py-1 font-medium text-amber-700 text-xs dark:bg-amber-900/40 dark:text-amber-200">
              Target{" "}
              {state.mission.research.leaderboardAnalysis.targetScore.toFixed(
                4
              )}
            </span>
          )}
...
function MutationBreakdown() {
  ...
  const entries = [
    ...
    {
      label: "Structural",
      value: mutations.structural,
      color: "bg-violet-500",
    },
    {
      label: "Hyperparam",
      value: mutations.hyperparameter,
      color: "bg-amber-500",
    },
      ...
      <div className="grid gap-3 md:grid-cols-4">
        {entries.map((entry) => (
          ...
          >
            ...
            <p className="mt-2 font-semibold text-xl text-zinc-900 dark:text-white">
              {entry.value}
            </p>
          </div>
        ))}
...
function Badge({
  ...
  return (
    <span className={cn("rounded-full px-2 py-0.5 font-medium", palette)}>
      {children}
    </span>
  );
}
