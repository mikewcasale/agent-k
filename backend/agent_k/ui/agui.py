...
class IntentClassifier:
    ...
    async def classify(self, message: str) -> MissionIntentResult:
        """Classify a message and extract mission intent.

        Args:
            message: User message text.

        Returns:
            Parsed mission intent and optional criteria.
        """
...
class ChatHandler:
    ...
    async def parse_mission_criteria(self, messages: list[dict[str, Any]]) -> MissionCriteria | None:
        """Parse mission criteria from chat messages.

        Args:
            messages: Chat message payloads.
                ...
                try:
                    with logfire.span("mission_execution", chat_id=chat_id, attempt=attempt + 1):
                        async with LycurgusOrchestrator() as orchestrator:
                            ...
                            failure_summary = _extract_failure_summary(orchestrator.state)
                            await emitter.emit(
                                "mission-complete",
                                {
                                    ...
                                    "evolution_failure_summary": failure_summary,
                                },
                            )
                    if attempt > 0:
                    ...
                    try:
                        ...
                        await emitter.emit(
                            "mission-complete",
                            {
                                ...
                                "evolutionFailureSummary": failure_summary,
...
def _extract_failure_summary(state: MissionState | None, *, limit: int = 5) -> dict[str, int]:
    if state is None or state.evolution_state is None:
        return {}
    summary = state.evolution_state.failure_summary or {}
    if not summary:
        return {}
