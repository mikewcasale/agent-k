...
class MissionPersistence(FileStatePersistence[MissionState, MissionResult]):
    ...
    def __init__(
        self, mission_id: str, checkpoint_dir: Path = CHECKPOINT_DIR, max_checkpoints: int = 10
    ) -> None:
    ...
    async def snapshot_node(
        self,
        state: MissionState,
        next_node: BaseNode[MissionState, Any, MissionResult],
    ) -> None:
    ...
    def _ensure_types(self) -> None:
        ...
        with _utils.set_nodes_type_context(
            [DiscoveryNode, ResearchNode, PrototypeNode, EvolutionNode, SubmissionNode]
        ):
    ...
    async def _cleanup_old_checkpoints(self) -> None:
        checkpoints = sorted(
            self.mission_dir.glob(f"{CHECKPOINT_PREFIX}*.json"),
