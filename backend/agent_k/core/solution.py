...
_KAGGLE_WORKING_PREFIX: Final[str] = "/kaggle/working"
_KAGGLE_WORKING_PATTERN: Final[re.Pattern[str]] = re.compile(r"/kaggle/working(?=/|$|[\"'\s)\],])")
...
def _normalize_kaggle_paths(code: str) -> str:
    if _KAGGLE_INPUT_PREFIX in code:
        code = code.replace(_KAGGLE_INPUT_PREFIX, ".")
    if _KAGGLE_WORKING_PREFIX in code:
        code = _KAGGLE_WORKING_PATTERN.sub(".", code)
    return code
